...
/*
  DEBUG: vì toàn bộ app được bọc trong IIFE nên biến "state"
  không có sẵn ở Console (window). Expose ra window để có thể
  gõ: window.LP.state.productionPlans["2026-08"]...
  Object.assign(state, ...) chỉ mutate object này, không tạo
  object mới, nên tham chiếu window.LP.state luôn đồng bộ.
*/
window.LP = window.LP || {};
window.LP.state = state;

/*
  DEBUG: các giá trị dưới đây là biến tạm trong module,
  không nằm trong "state" — cần getter "sống" (đọc lại
  mỗi lần truy cập) vì chúng bị gán lại (=) liên tục,
  không phải mutate tại chỗ như "state".
*/
Object.defineProperty(window.LP, "simulationViewLog", {
  get: () => simulationViewLog
});
Object.defineProperty(window.LP, "simulationViewConsumedMap", {
  get: () => simulationViewConsumedMap
});
Object.defineProperty(window.LP, "activeEntryLog", {
  get: () => getActiveEntryLog()
});
Object.defineProperty(window.LP, "projectedEndOfDay", {
  get: () => projectEndOfDay()
});
window.LP.recompute = () => recompute();
window.LP.renderAll = () => renderAll();
window.LP.saveState = () => saveState();
window.LP.advanceProductionTime = advanceProductionTime;
window.LP.capacity = capacity;
    ...
    .on(
      ...
      payload => {
        ...
        try{
          ...
          /*
            simTime/currentTick/entryLog KHÔNG phải là
            dữ liệu "cộng tác nhiều người sửa" — đó là
            đồng hồ/log đang chạy sống ở LOCAL. Bản ghi
            trên Supabase luôn là ảnh chụp cũ hơn vài giây
            (dù là do chính client này tự echo lại, hay do
            client khác gửi lên chậm hơn). Nếu để
            normalizeState() ghi đè các trường này, đồng hồ
            sẽ bị "kéo lùi" ngẫu nhiên rồi tự nhảy tiến lại
            => hiện tượng nhảy giờ linh tinh — xảy ra ở CẢ
            REAL TIME lẫn SIMULATION, nên phải giữ nguyên
            local cho cả 2 chế độ, không chỉ riêng real time.
          */

          const localSimTime = state.simTime;
          const localLastTimeSync = state.lastTimeSync;
          const localCurrentTick = state.currentTick;
          const localEntryLog = state.entryLog;
          const localActualThrough = state.actualThrough;

          normalizeState(payload.new.state);
          dayIndexReset();

          if(state.timeMode === "realtime"){
            const nowFix = new Date();
            state.simTime = nowFix.toISOString();
            state.lastTimeSync = nowFix.toISOString();
          }else{
            state.simTime = localSimTime;
            state.lastTimeSync = localLastTimeSync;
          }

          state.currentTick = localCurrentTick;
          state.entryLog = localEntryLog;
          state.actualThrough = localActualThrough;
...
/*
  TAKT THỰC TẾ CỦA TỪNG NGÀY

  Vì KHSX không có cột UPH riêng, UPH thực tế của
  từng ngày được suy ngược: UPH = Planned Qty / Working Hours.
  Nếu ngày đó không có đủ dữ liệu KHSX (chưa import,
  hoặc plannedQty = 0), dùng takt mặc định trong Cấu hình
  làm phương án dự phòng.
*/
function taktForDate(date){
  ...
  if(
    ...
    return (hours * 3600) / qty;

  }

  return state.config.takt;
...
/* =========================================================
   TÍNH GIỜ KẾT THÚC CA THEO SỐ GIỜ LÀM VIỆC THỰC TẾ
   (Working Hours KHÔNG bao gồm giờ nghỉ — nếu trong
   khoảng đang cộng dồn có break thì phải "nhảy qua"
   break đó, cộng thêm đúng phần thời gian nghỉ vào
   giờ kết thúc, để tổng thời gian LÀM VIỆC = hours.)
   ========================================================= */

function computePlannedShiftEnd(start, hours, breaks){
...
function getDayProductionWindow(date, config){
  ...
  */
  ...
  if(hours !== null){
    ...
    /*
      Working Hours trong KHSX là số giờ LÀM VIỆC
      THỰC TẾ, không tính giờ nghỉ. Nên "end" phải
      được tính bằng cách cộng dồn thời gian làm việc,
      nhảy qua các khoảng nghỉ (break) — chứ không phải
      cộng thẳng "hours" vào giờ bắt đầu ca.
    */
...
return {
  ...
  /*
    ...
    (đã cộng thêm thời gian nghỉ nằm
    trong ca để đủ đúng số giờ làm
    việc thực tế theo KHSX).
  */
  end,
...
/* =========================================================
   CỘNG DỒN THỜI GIAN SẢN XUẤT — CÓ THỂ VẮT QUA NHIỀU NGÀY

   Dùng để tính exitTime: xe cần đi qua đủ "capacity"
   khoang × takt giây SẢN XUẤT THỰC TẾ mới ra khỏi chuyền.
   Ban đêm / ngoài ca / ngày nghỉ / giờ nghỉ giữa ca đều
   KHÔNG được tính — chuyền chỉ "trôi" trong giờ làm việc.
   ========================================================= */

function advanceProductionTime(startDate, seconds, config){
  ...
  for(
    ...
    if(
      ...
      for(const br of sortedBreaks){
        ...
        if(br.end <= segCursor) continue;

        if(br.start > segCursor){

          const chunk =
            (br.start.getTime() -
             segCursor.getTime()) / 1000;
    ...
    // Sang ngày kế tiếp, tính lại window từ đầu ngày đó
    cursor =
...
/* =========================================================
   CỘNG DỒN THEO SỐ XE (KHÔNG PHẢI GIÂY) — VẮT QUA NHIỀU NGÀY

   Dùng riêng cho exitTime: hành trình 1 xe đi qua "capacity"
   khoang có thể vắt qua nhiều ngày có UPH/takt KHÁC NHAU
   (KHSX suy ngược UPH mỗi ngày = Planned Qty / Working Hours).
   Không thể quy đổi capacity ra "số giây" một lần duy nhất
   (như advanceProductionTime) vì mỗi ngày có thể có số giây/xe
   khác nhau — phải đếm XE, dùng đúng takt của ngày đang đi qua.
   ========================================================= */

function advanceProductionCars(startDate, carsToAdvance, config){
    ...
    if(
      ...
      for(const br of sortedBreaks){
        ...
        if(br.start > segCursor){
          ...
          const carsInChunk =
            Math.floor(chunkSeconds / takt);

          if(remainingCars <= carsInChunk){
            return new Date(
              segCursor.getTime() +
              remainingCars * takt * 1000
            );
          }

          remainingCars -= carsInChunk;
          segCursor = new Date(br.end);
      ...
      const carsInTail =
        Math.floor(tailSeconds / takt);

      if(remainingCars <= carsInTail){
        return new Date(
          segCursor.getTime() +
          remainingCars * takt * 1000
        );
...
/* =========================================================
   BẢNG TRA CỨU NGÀY → TỔNG SỐ XE CỘNG DỒN (CACHE)

   computeCurrentTick() và computeEntryTimeForTick() vốn
   phải DUYỆT LẠI TỪ ĐẦU (từ lineStartDate) mỗi lần gọi để
   tìm "tick này/ngày này nằm ở đâu" — càng mô phỏng xa,
   số ngày phải duyệt lại càng nhiều, gây giật/lag.

   Bảng này lưu sẵn từng ngày đã tính 1 LẦN DUY NHẤT, mở
   rộng dần về sau khi cần, KHÔNG bao giờ tính lại từ đầu.
   Phải xoá cache (dayIndexReset()) mỗi khi cấu hình
   (lineStartDate/shiftStart/breaks) hoặc KHSX thay đổi.
   ========================================================= */

let _dayIndexEntries = [];
let _dayIndexAnchorTime = null;

function dayIndexReset(){
  _dayIndexEntries = [];
  _dayIndexAnchorTime = null;
}

function dayIndexExtendWhile(anchor, config, shouldContinue){

  if(
    _dayIndexAnchorTime === null ||
    _dayIndexAnchorTime !== anchor.getTime()
  ){
    _dayIndexEntries = [];
    _dayIndexAnchorTime = anchor.getTime();
  }
  ...
  let cum =
    _dayIndexEntries.length
      ? _dayIndexEntries[_dayIndexEntries.length-1].cumAfter
      : 0;

  let cursor =
    _dayIndexEntries.length
      ? new Date(
          _dayIndexEntries[_dayIndexEntries.length-1]
  ...
  while(
    shouldContinue(_dayIndexEntries, cum) &&
    guard < safetyLimit
  ){
...
/*
  Tìm entry mà tick nằm trong đó
  (cumBefore < tick <= cumAfter).
  Nhị phân trên mảng đã sắp xếp theo ngày (cumAfter tăng dần).
*/
function dayIndexFindForTick(tick, anchor, config){

  dayIndexExtendWhile(
    anchor,
    config,
    (entries, cum) => cum < tick
  );

  let lo = 0;
  let hi = _dayIndexEntries.length - 1;

  while(lo <= hi){

    const mid = (lo + hi) >> 1;
    const e = _dayIndexEntries[mid];

    if(e.qty > 0 && tick > e.cumBefore && tick <= e.cumAfter){
      return e;
    }
...
/*
  Tìm entry của đúng 1 ngày cụ thể (để lấy cumBefore của
  ngày đó — dùng cho computeCurrentTick tính phần "hôm nay").
*/
function dayIndexFindForDate(date, anchor, config){
  ...
  dayIndexExtendWhile(
    ...
    (entries) =>
      entries.length === 0 ||
      fmtYMD(entries[entries.length-1].date) < key
  );

  for(const e of _dayIndexEntries){
    if(fmtYMD(e.date) === key){
      return e;
    }
  }
...
function computeCurrentTick(){
  ...
  /*
    ...
    Dùng bảng tra cứu để lấy tổng số xe của TẤT CẢ
    các ngày TRƯỚC hôm nay (đã tính sẵn, O(1)/O(log n)),
    chỉ tự tính riêng phần "hôm nay" (ngày đang dở).
  */
    ...
    if(now >= start){
      ...
      if(effectiveEnd > start){
        ...
        for(const br of breaks){
          ...
          const overlapStart =
            Math.max(start.getTime(), bs);

          const overlapEnd =
            Math.min(effectiveEnd.getTime(), be);
...
/* =========================================================
   TÍNH ENTRYTIME CHO 1 TICK BẤT KỲ (không cần tuần tự)

   Trích xuất từ vòng lặp "tìm ngày cho tick" bên dưới,
   để dùng lại y hệt cho cả entryTime (tick hiện tại) LẪN
   exitTime (tick hiện tại + capacity) — đảm bảo
   exitTime(xe N) === entryTime(xe N+capacity) TUYỆT ĐỐI,
   vì khi chuyền đã đầy, 1 xe vào chuyền ĐỒNG THỜI với
   đúng 1 xe ra chuyền, bất kể UPH ngày đó là bao nhiêu.
   ========================================================= */

function computeEntryTimeForTick(tick, anchor, config){
  ...
  const targetSeconds =
    foundIndex *
    taktForDate(foundDate);

  return advanceProductionTime(
    window.start,
    targetSeconds,
    config
  );
...
function syncEntryLog(
  ...
  for(
    ...
    /*
      entryTime của chính tick này.
    */

    const entryTime =
      computeEntryTimeForTick(
        tick,
        anchor,
        ctx.config
      );
    ...
    /*
      Tìm LOT tiếp theo còn xe để gán cho tick này,
      dựa trên consumedMap (đang tiêu thụ tới đâu
      trong từng LOT theo thứ tự hàng đợi).

      LƯU Ý: đây là logic đã bị thiếu trong bản trước
      (hàm nextAvailableLot tồn tại nhưng chưa từng
      được gọi) — khiến mọi tick mới sinh ra không có
      code/model/spec/exitTime/unitIndex/totalQty.
    */
    ...
    if(lot){
      ...
      /*
        exitTime CHÍNH LÀ entryTime của xe thứ
        (N + capacity) — vì khi chuyền đã đầy,
        1 xe vào chuyền đồng thời với đúng 1 xe
        ra chuyền, bất kể UPH ngày đó thế nào.
        Tính bằng ĐÚNG hàm dùng cho entryTime để
        đảm bảo 2 giá trị này không bao giờ lệch nhau.
      */

      const exitTick =
        tick + capacity(ctx.config);
      ...
      /*
        Hết LOT khả dụng: tick vẫn diễn ra
        (chuyền vẫn "chạy"), nhưng không có
        xe/LOT thực để gán — coi là ô trống.
      */
...
function findPlannedQtyRow(rows, dayRow, hoursRow){
  ...
  /*
    Không tìm thấy nhãn "Qty"/"Quantity"/"Số lượng":
    fallback về dòng NGAY DƯỚI dòng Working Hours,
    vì cấu trúc phổ biến của KHSX là
    Day -> (Weekday) -> Working Hours -> Planned Qty.
  */
...
if(month === currentMonth){
  ...
  */
  ...
  Object.assign(
    ...
    /*
      Archive TẤT CẢ các bản đang "current" của
      đúng tháng này — không chỉ đúng 1 dòng "previous"
      (version cao nhất). Nếu vì lý do gì đó có sẵn
      nhiều hơn 1 dòng "current" bị lạc trôi từ trước,
      cách archive theo "previous.id" cũ sẽ bỏ sót,
      khiến lần insert bản mới bị chặn bởi ràng buộc
      "chỉ 1 current / tháng" (production_plans_one_current_per_month_uq).
    */
